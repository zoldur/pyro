...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='pyro.conf'
CONFIGFOLDER='/root/.pyrocore'
COIN_DAEMON='pyrod'
COIN_CLI='pyro-cli'
...
function enable_firewall() {
  echo -e "Installing and setting up firewall to allow incomming connections on port ${GREEN}$COIN_PORT${NC}"
...
function get_ip() {
  ...
  if [ ${#NODE_IPS[@]} -gt 1 ]
    then
      echo -e "${GREEN}More than one IP found. Please type 0 to use the first IP, 1 for the second and so on...${NC}"
      INDEX=0
      for ip in "${NODE_IPS[@]}"
      do
        echo ${INDEX} $ip
...
if [[ $(lsb_release -d) != *16.04* ]]; then
  echo -e "${RED}Your server is not not running Ubuntu 16.04. Installation is cancelled.${NC}"
  exit 1
fi
...
function important_information() {
 ...
 echo -e "Please check ${RED}$COIN_NAME${NC} daemon is running with the following command: ${GREEN}systemctl status $COIN_NAME.service${NC}"
 echo -e "================================================================================================================================"
}
